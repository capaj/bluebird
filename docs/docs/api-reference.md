...
- [Promisification](#promisification)
    ...
    - [Promise.fromNode](#promise.fromnode)
    - [.nodeify](#.nodeify)
- [Timers](#timers)
    - [.delay](#.delay)
    - [.timeout](#.timeout)
...
}).all().spread(function(file1text, file2text) {
...
```js
Promise.all(Array<any>|Promise<Array<any>> input) -> Promise
```
...
```js
Promise.props(Array<any>|Promise<Array<any>> input) -> Promise
```
...
```js
Promise.any(Array<any>|Promise<Array<any>> input) -> Promise
```
...
```js
Promise.race(Array<any>|Promise<Array<any>> input) -> Promise
```
...
```js
Promise.some(
    Array<any>|Promise<Array<any>> input,
    int count
) -> Promise
```
...
```js
.map(
    function(any value, int index, int length) mapper,
...
Map an array, or a promise of an array, which contains promises or a mix of promises and values where `item` is the resolved value of a respective promise in the input array. If any promise in the input array is rejected the returned promise is rejected as well.
...
Reduce an array, or a promise of an array, which contains promises ` where `item` is the resolved value of a respective promise in the input array, and `total` is either the initial value, or the result of the previous iteration. If any promise in the input array is rejected the returned promise is rejected as well.
...
Reduce will call the reducer as soon as possible, this is why you might want to use it over `.all`.
...
```js
.filter(
    function(any value, int index, int length) filterer,
    [Object {concurrency: int=Infinity} options]
) -> Promise
```
...
```js
.each(function(any value, int index, int length) iterator) -> Promise
```
...
Promise.each(
    Array<any>|Promise<Array<any>> input,
    function(any value, int index, int length) iterator
) -> Promise
```

Map an array, or a promise of an array, which contains promises or a mix of promises and values where `item` is the resolved value of a respective promise in the input array serially in-order. If any promise in the input array is rejected the returned promise is rejected as well.
...
emailTemplates(templatesDir).then(function(template) {
    return Promise.fromNode(template.bind(null, "newsletter"), true)
